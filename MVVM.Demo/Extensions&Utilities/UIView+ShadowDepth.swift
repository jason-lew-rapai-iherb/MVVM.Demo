...
extension UIView {
    ...
    func applyShadow(horizontalOffset: Double, verticalOffset: Double, opacity: Float, radius: CGFloat, color: UIColor) {
    ...
    func applyShadow(horizontalOffset: Double, verticalOffset: Double, opacity: Float, radius: CGFloat) {
        applyShadow(horizontalOffset: horizontalOffset, verticalOffset: verticalOffset, opacity: opacity, radius: radius, color: UIColor.black)
    }
    ...
    func applyShadowDown(withDepth depth: Int, opacity: Float) {
        ...
        default:
            applyShadow(horizontalOffset: 0, verticalOffset: Double(depth), opacity: opacity, radius: CGFloat(depth))
        }
    }
    
    func applyShadowUp(withDepth depth: Int) {
        applyShadowUp(withDepth: depth, opacity: UIView.defaultShadowOpacity)
    }
    
    func applyShadowUp(withDepth depth: Int, opacity: Float) {
        let opacity: Float = opacity + (Float(depth) / 100)
        switch depth {
        case 0:
            hideShadow()
